  ...
  <main class="auth-page">
    <section class="auth-card">
      ...
      <form @submit.prevent="submit">
        ...
        <label v-if="mode !== 'forgot'">Password<input v-model="password" type="password" :autocomplete="mode === 'signup' ? 'new-password' : 'current-password'" minlength="8" required /></label>
        <label v-if="mode === 'signup'">Confirm password<input v-model="confirmPassword" type="password" autocomplete="new-password" minlength="8" required /></label>
      ...
      <nav class="auth-links">
        <RouterLink v-if="mode !== 'login'" to="/login">Sign in</RouterLink>
        <RouterLink v-if="mode !== 'signup'" to="/signup">Create account</RouterLink>
        <RouterLink v-if="mode !== 'forgot'" to="/forgot-password">Forgot password?</RouterLink>
      </nav>
    </section>
  </main>
</template>
